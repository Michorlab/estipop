...
test_estim = function(){  
  ...
  est = estimateBP(time = simdat$time, N = simdat$t1_cells_prev, initTime = simdat$prev_time, data = simdat$t1_cells, transitionList = transitionList, lower = c(0,0,0,0), upper = c(.5,.5,.5,.5), initial = c(.25,.25,.25,.25))
}