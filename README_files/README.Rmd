...
* [GNU Scientific Library](https://www.gnu.org/software/gsl/)
    ...
    + (Windows) download and extract the file [local###.zip](http://www.stats.ox.ac.uk/pub/Rtools/goodies/multilib/) and create an environmental variable LIB_GSL to add the directory (see notes about Windows installation below for more details).
...
## Objects in ESTIPop
ESTIpop uses a common representation of branching process models for simluation and estimation. A branching process model is represented by a `process_model` object containing one or more `transition` objects, and each `transition` object is associated with a `rate` object.

### rate Objects

A `rate` object represents the rate at which some event occurs, and may be a function of time and multiple other parameters. The constructor `rate(exp)` takes an R expression which describes how the rate depends on time and other parameters. For example:
...
It is important to note that the variable `t` and the `params` vector are not variables which we have defined previously in the program. The `rate` constructor acts on an _expression_ which encodes how the rate evolves with time and various other parameters. We will be able to evaluate this expression later when we plug in parameter values at a specific time. 

The `rate` constructor expects expressions which conform to the following subset of the R grammar, and will throw an error at other inputs.

\begin{center}
Table 1: Grammar for inputs into the \texttt{rate} expression
\end{center}
...
A `transition` object represents a type of birth or death event which can occur in the branching process model. A `transition` object has 3 attibutes: 

1. `parent`: The type of the parent which dies and is removed from the population when the transition occurs
2. `offspring`: The number of each type of offspring which are produced in the transition. This should be an `d`-length vector where `d` is the number of types in the population.
3. `rate`: A `rate` object (described above) governing how quickly this transition occurs
...
\begin{figure}
\begin{center}
\includegraphics{images/transition_new}\\
\end{center}
\caption{Example of a \texttt{transition} with \texttt{parent = 1} and \texttt{offspring = c(2,0)}. The blue circles represent individuals of type 1 and the green circles represent individuals of type 2.}
\label{fig:transition}
\end{figure}


## process_model Objects

A `process_model` object is a collection of `transition` objects which together make up a branching process system. A `process_model` object is constructed from a sequence of `transition` objects which can occur in the system. All of the `transition` objects must assume the same number of types, and the `process_model` constructor will throw an error if this is not the case.
...
Note that `model` still retains all of the time- and parameter-dependence of its constituent transitions. Also notice that, since `params[3]` was used in both the birth and death rate, this model has the constraint that birth and death rates decay at equal speed.

## Simulation

Armed with a `process_model` object, ESTIpop allows for the simulation of this system under various parameters values. ESTIpop uses C++ for high-performance simulation. In the case of time-dependent rates, ESTIpop parses the expressions and translates them into dynamically loaded C++ code.


The `branch` function is used to simulate the system specified by a `process_model` object. It's parameters are as follows:

\begin{center}
Table 2: Syntax for the \texttt{branch} function
\end{center}
|	Argument| Type | Description | Optional? | Default                                  
|-----------+-------------------+---------------------------------------------------------|
| `model` | `process_model` object| The system to simulate | No | N/A
| `params`| numeric vector| The parameters to plug into the system during simulation | No |N/A
| `init_pop`| numeric vector| The population at the start of the simulation (time 0) | No |N/A
| `time_obs`| numeric vector| The timepoints at which to record the state of the population |No|N/A
| `reps` | numeric scalar | The number of times to run the simulation | No | N/A
| `silent`| logical| Whether to silence intermediate printouts form the C++ simulator | Yes | FALSE
| `keep`| logical| Whether to keep logs files from the C++ simulation | Yes | FALSE
| `seed`| logical| A seed for the random number generator | Yes | NULL
...
We'll start by simulation the one-type birth-death model shown in Figure \ref{fig:birthdeath}.  In this model, a population of a single type experiences birth events, in which an individual from the population is chosen to replicate, and death events, in which an individual from the population is chosen for removal.  To test our estimation procedure, we begin by simulating data using functions available in ESTIpop.  We initiate the population with size 100 and allow it expand for 5 units of time with birth parameter 1 and death parameter 0.7.  Using the following code, we generate 1,000 samples from this process. 

\begin{figure}
\begin{center}
\includegraphics{images/birthdeath.PNG}\\
\end{center}
\caption{One-type birth-death model. Individuals give birth at rate $b(t)$, increasing the population by 1. Individuals die at rate $d(t)$, which decreases the population by 1.}
\label{fig:birthdeath}
\end{figure}
	
```{r sup1, echo = T, cache = TRUE, results = FALSE, warning=FALSE}
library(estipop)
...
`res` now contains a dataframe with the result of the simulation. We can now analyze the results of the simulation.

```{r sup2, echo = TRUE, cache = TRUE, results = T, eval = T}
...
We now consider the two-type process shown in Figure \ref{fig:bdm}.  Here, each of the two population types has separate birth and death rates, as well a mutation rate from the type 1 population to the type 2 population. We'll again simulate 1000 examples and compare the results with the theoretical moments.
\begin{figure}
\begin{center}
\includegraphics{images/birthdeathmutation.PNG}\\
\end{center}
\caption{Two-type birth-death-mutation model.  Each type has its own birth and death rate parameters in addition to a mutation event from type 1 to type 2.}
\label{fig:bdm}
\end{figure}
\vspace{0.1in}

```{r sup4, echo = TRUE, cache = TRUE, results = FALSE, eval = T}
library(estipop)
...
```{r sup5, echo = TRUE, cache = TRUE, results = T, eval = T}
...
Given `process_model` object and a set of datapoints generated from that model, ESTIpop can estimate the model's unknown parameter values. 


The `estimate` and `estimate_td` functions are used to estimate the parameters of `process_model` object. The arguments these function as as follows:


\begin{center}
Table 3: \texttt{estimate\_td} syntax. \texttt{N} is the number of observations in the dataset, \texttt{d} is the number of types in the population, and \texttt{m} is the number of parameters.
\end{center}
|	Argument| Type | Description | Optional? | Default                                  
|-----------+-------------------+---------------------------------------------------------|
| `model` | `process_model` object| The system whose parameters are being estimated | No | N/A
| `init_pop`| `N` x `d` matrix | The initial population in each of the observations in the dataset | No |N/A
| `final_pop`| `N` x `d` matrix| The final observed population in each observation | No |N/A
| `start_times`| `N` x 1 vector| The time at which each entry in `init_pop` was observed |No|N/A
| `end_times` | `N` x 1 vector| The time at which each entry in `final_pop` was observed | No | N/A
| `initial_ params`| `m` x 1 vector| The parameter values at which to initialize the estimation procedure | Yes | FALSE
| `lower`| `m` x 1 vector| Lower bounds on each parameter to be estimated | Yes | NULL
| `upper`| `m` x 1 vector| Uppose bounds on each parameter to be estimated | Yes | NULL
| `trace`| logical | Whether to enable output during optimization | Yes | F
  
The `estimate` function is a wrapper for the `estimate_td` function. `estimate` only requires the elapsed time between the initial and final obervations of each entry in the dataset, rather then the exact times of each observation. `estimate` can be used when the rates of a model do not depend on time. 
...
```{r estim1, echo = T, cache = TRUE, results = FALSE, warning=FALSE, eval = T}
library(estipop)
...
```{r estim3, echo = T, cache = TRUE, results = T, warning=FALSE, eval = T}
print(est$par)
```
...
```{r estim4, echo = T, cache = TRUE, results = F, warning=FALSE, eval = T}
time = seq(0,10,1) 
initial = c(100, 0)
...
```{r estim5, echo = T, cache = TRUE, results = T, warning=FALSE, eval = T}
print(est$par)
```
...
```{r estim6, echo = T, cache = TRUE, results = F, warning=FALSE, eval = T}
time = seq(0,10,1) 
initial = c(100, 0,0,0)
...
```{r estim7, echo = T, cache = TRUE, results = T, warning=FALSE, eval = T}
print(est$par)
```
