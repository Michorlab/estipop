...
test_that("Inavlid expressions are detected by the validator",{
  expect_equal(check_valid(quote(params[1] + t[1])), F)
  expect_equal(check_valid(quote(t1)), F)
  expect_equal(check_valid(quote(params[-1] + 2)), F)
  expect_equal(check_valid(quote(t*5 + params[-6])), F)
  expect_equal(check_valid(quote(params[abcd])), F)
  expect_equal(check_valid(quote(params[])), F)
  expect_equal(check_valid(quote(params[t])), F)
  expect_equal(check_valid(quote(params[1]*t[5])), F)
  expect_equal(check_valid(quote(params[1+1])), F)
  expect_equal(check_valid(quote(params[3] + params12)), F)
  expect_equal(check_valid(quote(params[[3]])), F)
  expect_equal(check_valid(quote("hi mom")), F)
  expect_equal(check_valid(quote(params[2] & t)), F)
  expect_equal(check_valid(quote(c(1))), F)
  expect_equal(check_valid(quote(exp(params[1],params[3]))), F)
  expect_equal(check_valid(quote(log(params[1],params[3], params[2]))), F)
  expect_equal(check_valid(quote(sin())), F)
})
